...
def extract_frames(video_path, frames_dir, overwrite=False, start=-1, end=-1, every=1):
    ...
    if every > 25 and len(frames_list) < 1000:  # this is faster for every > 25 frames and can fit in memory
        ...
        for index, frame in zip(frames_list, frames):  # lets loop through the frames until the end
            save_path = os.path.join(frames_dir, video_filename,video_filename+"{:010d}.png".format(index))  # create the save path
...
if __name__ == '__main__':
    # test it
    video_to_frames(video_path='/home/ariana.venegas/Documents/HangDai/ML703/datasets/12fpsPeriodic/periodic_legs.mp4', frames_dir='periodic_leg', overwrite=False, every=5)
